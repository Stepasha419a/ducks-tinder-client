...
export class UsersService {
    ...
    async savePicture(dto: SavePictoreDto, picture: Express.Multer.File): Promise<UserDto> {
        const fileName = this.filesService.savePicture(picture, dto.userId, dto.setting)
        ...
        const newUser = await this.userModel.findByIdAndUpdate(user._id, user, {new: true})
    ...
    async deletePicture(dto: DeletePictoreDto): Promise<UserDto> {
        ...
        if(!user.pictures[dto.setting]) throw new HttpException('Pictures were not found', HttpStatus.BAD_REQUEST);

        if(dto.setting === 'gallery') {
            if(!user.pictures.gallery.includes(dto.pictureName)) throw new HttpException(`Picture with such a name ${dto.pictureName} was not found`, HttpStatus.BAD_REQUEST);
            const fileName = this.filesService.deletePicture(dto.pictureName, dto.userId, dto.setting)
    ...
    async createPair(userPairDto: UserPairDto): Promise<UserDto> {
        ...
        if(!user.pairs.includes(userPair._id)) {
            ...
            throw new HttpException('Pair with such an id already exists', HttpStatus.BAD_REQUEST)
        }
    ...
    async deletePair(userPairDto: UserPairDto): Promise<UserDto> {
        ...
        if(user.pairs.includes(userPair._id)) {
            ...
            throw new HttpException('Pair with such an id was not found', HttpStatus.BAD_REQUEST)
        }
